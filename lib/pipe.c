...
pipe(int pfd[2])
{
	...
	// if (debug)
	{
		cprintf("IN PIPE fd0 data: va is [%08x]\n", uvpt[PGNUM(va)]);
		cprintf("IN PIPE fd1 data: va is [%08x]\n", uvpt[PGNUM(fd2data(fd1))]);
		cprintf("IN PIPE for [%08x]\n", sys_getenvid());
	}
