...
int 
serve_type(envid_t envid, struct Fsreq_type *req)
{
	int r;
	...
	if ((r = file_type(path)) < 0)
	{
		if (debug)
			cprintf("Error in serve_type: %i\n", r);
		return r;
	}
	return r;
}
...
int
serve_create_fifo(envid_t envid, struct Fsreq_fifo *req)
{
	int r;
	char path[MAXPATHLEN];
	struct File *f;
	...
	r = file_create(path, &f, FTYPE_FIFO);
	if (r < 0)
	{
		return r;
	}
...
fshandler handlers[] = {
	...
	[FSREQ_FIFO] = 		(fshandler)serve_create_fifo,
	[FSREQ_TYPE] = 		(fshandler)serve_type,
};
...
serve(void)
{
	...
	while (1) {
		...
		}// else if (req == FSREQ_FIFO) {
		// 	r = serve_create_fifo(whom, (struct Fsreq_fifo*)fsreq);
		// } 
