...
sys_exofork(void)
{
	...
	int err;
	err =  env_alloc(&new_proc, curenv->env_id);
	if (err != 0) 
	{
		return err;
	}

	new_proc->env_status = ENV_NOT_RUNNABLE;
	new_proc->env_tf = curenv->env_tf;
	new_proc->env_parent_id = curenv->env_id;
	new_proc->env_tf.tf_regs.reg_eax = 0;//result of syscall

	return new_proc->env_id;
}
...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	// address!
	struct Env *new_proc;
	if (envid2env(envid, &new_proc, 1))
		return -E_BAD_ENV;
	user_mem_assert(new_proc, tf, sizeof(*tf), 0);
	new_proc->env_tf = *tf;
	return 0;
}
...
sys_env_set_pgfault_upcall(envid_t envid, void *func)
{
	...
	new_proc->env_pgfault_upcall = func;
	return 0;
}
...
sys_page_unmap(envid_t envid, void *va)
{
	...
	if (((uintptr_t)va >= UTOP) ||	//-E_INVAL if va >= UTOP
		((uintptr_t)va % PGSIZE))	//va is not page-aligned
	{
		err = -E_INVAL;
		return err;
	}

	page_remove(new_proc->env_pgdir, va);
