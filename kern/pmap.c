...
page_init(void)
{
	...
	// free pages!
	size_t i;
	//0 элемент занят, это даже в подсказке сверху написано и в лабе
	//Mark physical page 0 as in use.
	//     This way we preserve the real-mode IDT and BIOS structures
	//     in case we ever need them
	// поэтому меняем в цикле i = 0 на i = 1


	// теперь почему npages_basemem:
	//2) The rest of base memory, [PGSIZE, npages_basemem * PGSIZE)
	//     is free.
	//Первая страница соответствует структурам BIOS и IDT. 
	//Остальная часть базовой памяти свободна.(указания в лабе)
	...
	// (EXTPHYSMEM - IOPHYSMEM) / 1024 = 384 кб
	...
	//расширенная память, 
	//часть которой занята (уже выделена с помощью boot_alloc()
	//поэтому мы ищем номер первой свободной страницы
	//(boot_alloc(0) --> Если n=0, возвращает адрес следующей свободной страницы, ничего не выделяя)
	//находим PGNUM(PADDR(boot_alloc(0))) и дальше идем по всем оставшимся страницам
...
//По указателю на каталог страниц pgdir, возвращает записть pte_t в таблице страниц для адреса va
pte_t *
...
//Отображает виртуальные адреса [va, va+size) по физическим адресам [pa, pa+size) в каталоге страниц pgdir
static void
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	//LAB 7: Your code here
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	//LAB 7: Your code here
	// Fill this function in
	//0 - потому что нам не надо выделять новую таблицу страниц
...
//Если страница по этому адресу отсутствует, ничего не делает. 
//Счетчик ссылок должен быть уменьшен, а если он достиг 0, страница должна быть освобождена.
void
page_remove(pde_t *pgdir, void *va)
{
	...
	//pte_store будет хранить адрес страницы
	//Если страница по этому адресу отсутствует, ничего не делаем
	if (!page) return;
	//Уменьшаем счетчик ссылок, а если он станет равным 0, то 
	//в page_decref() будет вызвана page_free()
	page_decref(page);
	//
...
//Отображает физическую страницу pp по адресу va. Флаги должны быть установлены в perm|PTE_P. 
//Если страница по этому адресу уже отображена, она должна быть удалена с помощью page_remove(). 
//Если необходимо, таблица страниц должна быть выделена и добавлена в pgdir. Счетчик ссылок должен быть увеличен.
int
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	...
	pte_t *pte = pgdir_walk(pgdir, va, 0);
  	// Increment ref-count here so that we cannot accidentally free a page that's mapped again to the same virtual address
  	pp->pp_ref++;
	...
	// We have to make sure the page table entry has this permission!
	*pte = pa | perm | PTE_P;
	return 0;
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
		...
		user_mem_check_addr += PGSIZE)
	{
		...
		page_table_entry = pgdir_walk(page_dir_entry, (void *)user_mem_check_addr, 0);
