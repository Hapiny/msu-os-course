...
env_setup_vm(struct Env *e)
{
	...
	//в каталоге NPDENTRIES = 1024 записей
...
env_alloc(struct Env **newenv_store, envid_t parent_id)
{
	...
	//LAB 3: Your code here.
	e->env_tf.tf_esp = 0x00210000 + 0x2000 * (e - envs);//(e-envs) = индекс в массиве envs 
	//размер страницы 4096 байт
	//4096 = 0х1000, а нам достаточно 2 страница под стек для одного процесса
#else
...
bind_functions(struct Env *e, struct Elf *elf)
{
	...
	{
		if (strcmp(&sh_strtab[sh->sh_name], ".strtab") == 0) 
		{
			strtab = (char *) elf + sh->sh_offset;
		}
		else
		if (strcmp(&sh_strtab[sh->sh_name], ".symtab") == 0) 
		{
...
env_destroy(struct Env *e)
{
	...
	{
		curenv = NULL;
		sched_yield();//передаем управление обратно планировщику
	}
	/*
